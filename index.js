												...
												res.on('end', () => {
															...
															if (!coverArtUrl) {
																const options = {
																	...
																	path: /2.0/ ? method = track.getInfo & api_key = YOUR_LAST_FM_API_KEY & artist = $ {
																...
																const req = http.get(options, (res) => {
																	...
																	res.on('end', () => {
																		if (res.statusCode !== 200) {
																			console.error(Error sending request to Last.fm API: status code $ {
																				res.statusCode
																			});
																			return;
																		}
															...
															app.get('/lyrics', (req, res) => {
																		res.send( < h1 > $ {
																				title
																			} < /h1> <h2>by ${artist}</h2 > < img src = "${coverArtUrl}"
																			alt = "Cover art for ${title}" > < pre > $ {
																				lyrics
																			} < /pre> );
																		});
